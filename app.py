...
app.layout = html.Div(children=[
    html.Div(
        children=[
            html.H2(
                children="Bitcoin Leveraged Trading Backtest Analysis", className='h2-title'),
        ],
                        ...
                        children=[
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    html.P(
                                        'Strategy Returns', className="twelve columns indicator_text"),
                                ]
                            ),
                            html.Div(
                                ...
                                className="two columns indicator pretty_container",
                                children=[
                                    html.P(id="market-returns",
                                           className="indicator_value"),
                                    html.P(
                                        'Market Returns', className="twelve columns indicator_text"),
                                ]
                            ),
                            html.Div(
                                ...
                                className="two columns indicator pretty_container",
                                children=[
                                    html.P(id="strat-vs-market",
                                           className="indicator_value"),
                                    html.P('Strategy vs. Market Returns',
...
def filter_df(df, exchange, leverage, start_date, end_date):
    dff = df[(df['Exchange'] == exchange) & (df['Margin'] == int(leverage)) & (
...
def calc_returns_over_month(dff):
    ...
    dff_group = dff.groupby('YearMonth')

    for name, group in dff_group:
...
def update_date_range(exchange, leverage):
    df2 = df[(df['Exchange'] == exchange) & (df['Margin'] == int(leverage))]
...
def update_pnl_vs_trade_type(exchange, leverage, start_date, end_date):
    return {
...
def pnl_vs_trade_type(exchange, leverage, start_date, end_date):
    df3 = filter_df(df, exchange, leverage, start_date, end_date)

    df3 = df3[['Entry time', 'Trade type', 'Pnl (incl fees)']]
    data = []

    for name, group in df3.groupby('Trade type'):
        data.append(
            go.Bar(y=group['Pnl (incl fees)'],
                   x=group['Entry time'], name=name, orientation='v')
        )
    return data
